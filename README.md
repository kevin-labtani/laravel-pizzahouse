...
- 1st way
  in the routes file `web.php` add on a middleware on the route we want protected eg. `Route::get('/pizzas', 'PizzaController@index')->middleware('auth');`
- 2nd way
  in the `PizzaController.php`, add the middleware in the constructor to rpotect all the routes on the controller
  ```php
...
in the routes file `web.php`, `Auth::routes();` are all the routes that were created by Laravel, you can see them all by running `php artisan route:list`

we want to disable the register routes so that a random client can't just creeate an account, we do that by passing an array:
